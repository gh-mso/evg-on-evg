var Evergage = (function (exports) {
  ...
  var INVALID_USER_OR_ACCOUNT_STRINGS = ['null', 'undefined', 'false', '0', '[object Object]']; // Sitemap re-init catalog rejection

  var SITEMAP_REINIT = "sitemap_reinit"; // Event receiver paths
  ...
  var setCookieHash = function setCookieHash(account, dataset, cookieDomain) {
    cookieHash = sha1$1("".concat(account, ".").concat(dataset, ".").concat(generateCookieDomainHash(cookieDomain))).slice(0, 4);
  };
      ...
      value: function send(payload) {
        ...
        return new Promise(function (resolve, reject) {
          ...
          xhr.onerror = function () {
            reject(createCustomError("Response error: ".concat(xhr.responseText), {
              status: this.status,
              statusText: xhr.statusText
            }));
          };
  ...
  var buildCart = function buildCart(lineItemConfig, itemKey) {
    return new Promise(function (resolve, reject) {
      ...
      resolveAndCleanValue(lineItemConfig, "lineItems", false, true).then(function (data) {
        var resolvedCart = {
          complete: {
            Product: []
          }
        ...
        var lineItems;
        var resolvedConfig = data;

        if (Object.keys(resolvedConfig).length === 0) {
          resolve(resolvedCart);
        }

        lineItems = resolveAndCleanValue(resolvedConfig, "lineItems", true, false);
        resolvedCart.complete['Product'] = lineItems;
        resolve(resolvedCart);
      }).catch(function (e) {
        Signals.onFireException.emit(new Error("Cart Config was rejected"), 'Site-wide Javascript');
      });
    });
  };

  var resolveAndCleanValue = function resolveAndCleanValue(value, key, clean, buildAttributes) {
    try {
      var resolvedValue;
      sitemapState.config.currentKey = key;

      if (value) {
        if ((key === "dimensions" || key === "lineItems" || key === "attributes") && buildAttributes) {
          resolvedValue = buildNestedAttributes(value, key);
        } else {
      ...
      return clean ? cleanValue(key, resolvedValue, sitemapState.config.settings.canonicalizeIds) : resolvedValue;
    } catch (e) {
  ...
  var setItemValue = function setItemValue(value, key, item) {
    if (value && _typeof(value) === "object" && typeof value.then === "function") {
      Signals.onFireException.emit(new Error("getValue for ".concat(key, " failed on ").concat(sitemapState.result.matchedConfig.name, " while evaluating custom function. ").concat(key, " cannot be a Promise.")), 'Site-wide Javascript');
  ...
  var buildItem = function buildItem(itemConfig, itemType) {
    return new Promise(function (resolve, reject) {
      ...
      for (var _i = 0, _idKeys = idKeys; _i < _idKeys.length; _i++) {
        var key = _idKeys[_i];
        var resolvedValue = resolveAndCleanValue(itemConfig[key], key, false, true);
      ...
      Promise.all(catalogPromises).then(function (values) {
        ...
        values.forEach(function (value, index) {
          ...
          var resolvedValue = resolveAndCleanValue(value, currentKey, true, false);
        ...
        Object.keys(itemConfig).forEach(function (key) {
          ...
          var resolvedValue = resolveAndCleanValue(valueFromConfig, key, true, true);
        ...
        Signals.onFireException.emit(new Error("Catalog Config was rejected"), 'Site-wide Javascript');
      });
    });
  };
  var buildOrder = function buildOrder(orderConfig) {
    return new Promise(function (resolve, reject) {
      ...
      orderConfigKeys.forEach(function (key) {
        ...
        var resolvedValue = resolveAndCleanValue(valueFromConfig, key, false, true);
      ...
      Promise.all(orderPromises).then(function (values) {
        ...
        values.forEach(function (value, index) {
          ...
          var resolvedValue = resolveAndCleanValue(value, orderConfigKey, true, false);
          order[orderConfigKey] = resolvedValue;
        });
        resolvedOrder["Product"] = order;
        resolve(resolvedOrder);
      }).catch(function (e) {
        Signals.onFireException.emit(new Error("Order Config was rejected"), 'Site-wide Javascript');
      });
    });
  };
  ...
  var buildMultipleLineItem = function buildMultipleLineItem(multipleLineItem) {
    ...
    var _loop = function _loop() {
      ...
      if (value.length != numItems) {
        Signals.onFireException.emit(new Error("Resolving multiple values for ".concat(key, " failed on ").concat(sitemapState.result.matchedConfig.name, ". ").concat(key, " does not have the same number of values as _id")), 'Site-wide Javascript');
      } else {
  ...
  var buildNestedAttributes = function buildNestedAttributes(nestedAttributeConfig, key) {
    ...
    nestedAttributeKeys.forEach(function (nestedKey) {
      ...
      var resolvedNestedAttributeValue = resolveAndCleanValue(nestedAttributeValue, nestedKey, false, true);
      nestedAttributes[nestedKey] = resolvedNestedAttributeValue;
    });

    if (key === "lineItems" || key === "dimensions") {
      return Promise.all(Object.values(nestedAttributes)).then(function (values) {
        values.forEach(function (value, index) {
          var key = nestedAttributeKeys[index];
          var resolvedValue = resolveAndCleanValue(value, key, false, false);
  ...
  var handleCatalog = function handleCatalog(pageConfig) {
    if (isCartConfig(pageConfig)) {
      return processCartConfig(pageConfig).then(function (data) {
        ...
        sitemapState.result.currentPage.cart = data;
      });
      ...
      return processOrderConfig(pageConfig).then(function (data) {
        ...
        sitemapState.result.currentPage.order = data;
      });
    } else {
      return processCatalogConfig(pageConfig).then(function (data) {
        sitemapState.result.currentPage.catalog = data;
      });
    }
  };
  ...
  var processCartConfig = function processCartConfig(pageConfig) {
    ...
    if (cartConfig.complete) {
      ...
      for (var itemKey in completeCartConfig) {
        ...
        if (itemKey !== "Product") {
          Signals.onFireException.emit(new Error('Item type must be Product'), 'Sitewide Javascript');
          return Promise.resolve(cartPayload);
        }
